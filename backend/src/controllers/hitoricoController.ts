...
const HistoricoController = {
  create: (req: Request, res: Response) => {
    const { data_historico, ID_animal, observacoes } = req.body;
    HistoricoRepositorie.create(data_historico, ID_animal, observacoes, (err, result) => {
  ...
  findById: (req: Request, res: Response) => {
    ...
    HistoricoRepositorie.findById((ID_historico), (err, historico) => {
      if (err) return res.status(500).json({ error: "Erro no servidor" });
      if (!historico) return res.status(404).json({ error: "Historico não encontrado" });
      res.status(200).json(historico);
    });
  },
  update: (req: Request, res: Response) => {
    const ID_historico: any = (req.params.ID_historico);
